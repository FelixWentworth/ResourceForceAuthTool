...
angular.module("story").component("storyTreeSceneEditor", {
	...
	controller: ["StoryEditorService", function (StoryEditorService) {
		...
		ctrl.doesntContain = function (types) {
			...
			for (var i in types) {
				if (Array.containsWhere(ctrl.scene.elements, function (e) {
					return e._type == types[i];
				})) {
		...
		ctrl.addChoice = function () {
			var choiceCount = 0;
			ctrl.scene.elements.forEach(function (e) {
				return choiceCount += e._type == "Choice" ? 1 : 0;
			});
		...
		function checkValidity() {
			ctrl.isThisSceneLevelComplete = Array.containsWhere(ctrl.scene.elements, function (e) {
				return e._type == "End" || e._type == "Choice";
			});
			ctrl.form.$setValidity("incomplete", ctrl.isThisSceneLevelComplete);
		}
	}]
});
//# sourceMappingURL=story-tree-scene-editor.component.js.map
