...
angular.module("story").factory("StoryStorageService", ["$http", "$q", function ($http, $q) {
	var service = {};
	...
	function getStories() {
		...
		$http.get('../api' + '/story').then(function (story) {
	...
	function getStory(id) {
		...
		$http.get('../api' + '/story/' + id).then(function (story) {
			storiesPromise.resolve(story.data);
		});
	...
	service.getById = function (id) {
		return getStory(id).then(function (story) {
			if (story != null) {
				return story;
			}
...
//# sourceMappingURL=story-storage-service.factory.js.map
