...
angular.module("story").factory("StoryEditorService", function () {
	var service = {};
	...
	function addCharacterReactions(subscene) {
		service.getCharacters().forEach(function (c) {
			...
			if (subscene.action._type == "Character" && subscene.action.name == c.name) {
				...
				character = Array.singleOrNull(subscene.reactions, function (reaction) {
					return reaction._type == "Character" && reaction.name == c.name;
				});
			}

			if (character != null) {
				emotion = Array.singleOrNull(character.elements, function (element) {
					return element._type == "Emotion";
				});
			} else {
...
//# sourceMappingURL=story-editor-service.factory.js.map
